...
// NewWithTargetIP creates a new Client instance that connects directly to the given robot IP.
func NewWithTargetIP(l *logger.Logger, targetIP string) (*Client, error) {
	ip := net.ParseIP(targetIP)
	if ip == nil {
		return nil, fmt.Errorf("invalid target IP: %s", targetIP)
	}

	c, err := new(l, 0, connection.TypeRouter, module.TypeAllButGamePad)
	if err != nil {
		return nil, err
	}

	c.connectionModule.SetTargetIP(ip)
	return c, nil
}
