...
type Gimbal struct {
	*internal.BaseModule
	...
	attitudeLock sync.RWMutex
	attitude     *value.GimbalAttitude
}
...
// Attitude returns the latest known attitude of the gimbal.
func (g *Gimbal) Attitude() *value.GimbalAttitude {
	g.attitudeLock.RLock()
	defer g.attitudeLock.RUnlock()
	if g.attitude == nil {
		return nil
	}
	cpy := *g.attitude
