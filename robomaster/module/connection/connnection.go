...
// SetTargetIP sets a specific IP to connect to directly, bypassing broadcast discovery.
func (c *Connection) SetTargetIP(ip net.IP) {
	c.targetIP = ip
}
...
func (c *Connection) Start() error {
	...
	if c.targetIP != nil {
		ip = c.targetIP
		c.f.SendACK(ip, c.appID)
	} else if c.typ == TypeRouter {
